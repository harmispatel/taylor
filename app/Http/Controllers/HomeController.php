...
class HomeController extends Controller
{
    ...
    public function measurer_appointment_accept_reject($id, $status)
    {
        ...
        $appointment = RequestAppointment::where(['measurer_id' => Auth::user()->id, 'id' => decrypt($id)])->first();
        // dd($appointment);
        $appointmentStatus = 0;
        $msg = '';
        if($status == 'accept') {
            ...
            $payment_url = 'https://www.paypal.me/taylormade/'.$appointment->measurer_commission;
            ...
            $content = "Hello, $name! This is Paypal Payment Link to Pay Commission to Measurer. " .$payment_url;
             ...
             Mail::send([], [], function ($message) use ($email, $content) {
                $message->to($email);
                $message->subject('Paypal Payment Link');
...
//    }
    ...
    public function all_model_gallery(Request $request)
    {
        ...
        if(isset($request->product_category) || isset($request->target_id) || isset($request->brand) || isset($request->user_id) || isset($request->size)  ){
            if(!empty($pCategory)){
            ...
            if(!empty($size)){
                $imagesPath = Upload::whereIn('id', $imagesId)->whereHas("model_details",function($query) use ($size){
